...
func (c *Trade) GetOrderDetail(req requests.OrderDetails) (response responses.OrderDetail, err error) {
	p := "/api/v5/trade/order"
	m := okex.S2M(req)
...
//
// `iceberg` order and `twap` order just supported on credit trading
//
...
//
// # Only released on credit trading
//
